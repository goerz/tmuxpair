...
def _mock_check_output(check_output_log):
    def mocked_check_output(*args, **kwargs):
        ...
        if cmd == 'tmux attach -t pair':
            raise subprocess.CalledProcessError(
                returncode=1, cmd=cmd, output="No existing session"
            )
...
def test_tmuxpair(tmpdir, authorized_keys, guest_key, monkeypatch, sig):
    ...
    monkeypatch.setattr(
        subprocess, "check_output", _mock_check_output(check_output_log)
    )
    ...
    p = Process(
        target=runner.invoke, args=(tmuxpair.main,), kwargs={'args': args}
    )
    p.start()
...
def test_tmuxpair_allow_port_forwarding(
    tmpdir, authorized_keys, guest_key, monkeypatch, sig
):
    shutil.copy(authorized_keys, authorized_keys + ".orig")
    check_output_log = os.path.join(str(tmpdir), "check_output.log")
    ...
    with open(authorized_keys) as in_fh:
        ...
        ssh-rsa  user@host2
        ssh-rsa  user@@host3
        command="tmux attach -t pair" ssh-rsa  guest@host
        '''
            ).strip()
        )
    if sig is not None:
...
def test_process_error(authorized_keys, guest_key, monkeypatch):
    def mocked_check_output(*args, **kwargs):
        cmd = " ".join(args[0])
        raise subprocess.CalledProcessError(
            returncode=1, cmd=cmd, output="test failure"
        )
...
def test_authorized_keys(tmpdir):
    ...
    key = sshkeys.Key.from_pubkey_line(
        r'command="/home/git/gitolite/src/gitolite-shell beXX",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty ssh-rsa  uk008XX7@XXXXiyot'
    )
    assert key in ak1
